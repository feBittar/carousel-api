...
export function getContentImageHtml(data: ModuleData, context?: RenderContext): string {
  ...
  // Resolve URLs to absolute (even if empty, to show placeholder)
  const absoluteUrl = contentImage.url ? resolveUrl(contentImage.url, context?.baseUrl) : '';
  const absoluteUrl2 = contentImage.url2 ? resolveUrl(contentImage.url2, context?.baseUrl) : '';

  // Warn if no URLs provided
