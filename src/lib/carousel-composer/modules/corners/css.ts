 ...
 */
function generateDuoCornerCSS(corners: Corner[], baseUrl?: string): string {
  let css = '';
  ...
  corners.forEach((corner, index) => {
    ...
    if (corner.type === 'svg') {
      ...
      const svgUrl = corner.svgUrl || '';

      // Only generate CSS if svgUrl is valid
      if (svgUrl && svgUrl.trim() !== '' && svgUrl !== 'none') {
        // Convert relative URLs to absolute URLs
        const absoluteSvgUrl = resolveUrl(svgUrl, baseUrl);
    ...
    .corner-${cornerNum}-s1 .corner-svg-mask,
    .corner-${cornerNum}-s2 .corner-svg-mask {
      width: ${svgWidth};
      height: ${svgHeight};
      background-color: ${svgColor};
      -webkit-mask-image: url('${absoluteSvgUrl}');
      mask-image: url('${absoluteSvgUrl}');
...
export function getCornersCss(data: ModuleData, context?: RenderContext): string {
  ...
  if (isDuo) {
    ...
    css += generateDuoCornerCSS(corners, baseUrl);
  } else {
    ...
    .corner-${cornerNum} .corner-svg-mask {
      width: ${svgWidth};
      height: ${svgHeight};
      background-color: ${svgColor};
      -webkit-mask-image: url('${absoluteSvgUrl}');
