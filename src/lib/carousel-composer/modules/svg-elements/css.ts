...
export function getSvgElementsCss(data: ModuleData, context?: RenderContext): string {
  ...
  let css = `
    ...
    .svg-element {
      position: absolute;
      /* SVG will be applied as mask-image, color via background-color */
  ...
  for (let i = 0; i < svgElements.length; i++) {
    ...
    // Use background-color for SVG color (works with mask-image)
    const backgroundColor = svg.color || '#ffffff';
    ...
    .svg-element-${svgNum} {
      ...
      -webkit-mask-image: url('${svg.svgUrl}');
      mask-image: url('${svg.svgUrl}');
