 ...
 * Uses a div with mask-image to allow precise color control via background-color
 */
function getSvgElementHtml(svgUrl: string, svgNum: number, enabled: boolean, baseUrl?: string): string {
  ...
  // Use div with inline style for mask-image (URL will be set via CSS class)
  return `<div class="svg-element svg-element-${svgNum}" data-svg-url="${absoluteUrl}"></div>`;
}
